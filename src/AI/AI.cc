...
AI::AI(plugin::Color color)
    : Player(color), opponent_color_(!color), best_move_(nullptr), board_()
{
...
int AI::evaluation_function(const ChessBoard &board)
{
  ...
  bonus_pos *= 0.2;
  king_tropism *= 0.2;
  int total = piece_material + material_bonus + bonus_pos + king_tropism - 50 * pawn_formation - king_file_malus;
